// supabase/functions/student-add-plant/index.ts
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { towerId, teacherId, name, port_number, quantity, planted_at } = await req.json()

    if (!towerId || !teacherId || !name) {
      throw new Error("Tower ID, Teacher ID, and plant name are required.");
    }
    ...
    // --- INSERT DATA ---
    // If the check passes, safely insert the plant data.
    ...
    return new Response(JSON.stringify({ message: "Plant added successfully" }), {
